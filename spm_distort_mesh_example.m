...
%%%%%  NOW PLOT RESULTS %%%%%%%%%%%
figure;
meanallF=squeeze(mean(allF_vals));
stderrallF=squeeze(std(allF_vals))./sqrt(size(allF_vals,1)-1);
...
set(gca,'FontSize',18)
figure;
meanallR2=squeeze(mean(allR2));
stderrallR2=squeeze(std(allR2))./sqrt(size(allR2,1)-1);
